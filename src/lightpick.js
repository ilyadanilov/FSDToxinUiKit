...
(function (root, factory) {
    ...
    renderMonthsList = function(date, opts)
    {
        ...
        for (var idx = 0; idx < 12; idx++) {
            d.set('month', idx);
            let mnth = d.toDate().toLocaleString(opts.lang, { month: 'long' });
            var option = document.createElement('option');
            option.value = d.toDate().getMonth();
            option.text = mnth.charAt(0).toUpperCase() + mnth.slice(1);
    ...
    renderCalendar = function(el, opts)
    {
        ...
        for (var i = 0; i < opts.numberOfMonths; i++) {
            ...
            for (var w = opts.firstDay + 4; w < 7 + opts.firstDay + 4; ++w) {
                let dayName = weekdayName(opts, w, true);
                html += '<div class="lightpick__day-of-the-week" title="' + weekdayName(opts, w) + '">' + dayName.charAt(0).toUpperCase()+dayName.slice(1) + '</div>';
            }
    ...
    Lightpick = function(options)
    {
        ...
        if (opts.footer) {
            ...
            if (opts.footer === true) {
                html += '<button type="button" class="btn btn-shade lightpick__reset-action">' + 'очистить' + '</button>';
                html += '<div class="lightpick__footer-message"></div>';
                html += '<button type="button" class="btn lightpick__apply-action">' + 'применить' + '</button>';
            }
            else {
                html += opts.footer;
            }
